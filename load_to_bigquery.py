...
from Bank_Statement_Analyser import detect_bank, extract_transactions, clean_and_enrich
...
# bank_name + statement_period_* are added for multi-bank tracking.
SCHEMA = [
    ...
    bigquery.SchemaField("statement_period_start","DATE"),
    bigquery.SchemaField("statement_period_end",  "DATE"),
    bigquery.SchemaField("_loaded_at",            "TIMESTAMP", mode="REQUIRED"),
]


def to_bq_rows(rows: list[dict], bank_name: str) -> list[dict]:
...
def main():
    ...
    print(f"  Bank: {bank} | {len(rows)} transactions | {min(dates)} → {max(dates)}")
